...
public class MultiLanguageUtils {
    ...
    public final static String LANGUAGE_IW = "iw";
    ...
    public final static String AREA_ZH = "ZH";
    //缓存Key
     ...
     * @param area     地区
     */
    public static void changeLanguageAndArea(Context context, String language, String area) {
        if (TextUtils.equals(LANGUAGE_AUTO, language) && TextUtils.equals(AREA_AUTO, area)) {
            //如果语言和地区都是空，那么跟随系统s
            PreferencesHelper.putStringValue(HealthApplication.getAppViewModel().getApplication(), SP_LANGUAGE, LANGUAGE_AUTO);
            PreferencesHelper.putStringValue(HealthApplication.getAppViewModel().getApplication(), SP_COUNTRY, AREA_AUTO);
        } else {
    ...
    public static void changeLanguage(Context context, String language) {
        if (TextUtils.equals(LANGUAGE_AUTO, language)) {
            //如果语言和地区都是空，那么跟随系统s
            PreferencesHelper.putStringValue(HealthApplication.getAppViewModel().getApplication(), SP_LANGUAGE, LANGUAGE_AUTO);
            PreferencesHelper.putStringValue(HealthApplication.getAppViewModel().getApplication(), SP_COUNTRY, AREA_AUTO);
        } else {
    ...
    public static boolean isSameWithSetting(Context context) {
        ...
        String sp_language = PreferencesHelper.getSharedPreferences(HealthApplication.getAppViewModel().getApplication()).getString(SP_LANGUAGE, MultiLanguageUtils.LANGUAGE_AUTO);
        String sp_country = PreferencesHelper.getSharedPreferences(HealthApplication.getAppViewModel().getApplication()).getString(SP_COUNTRY, MultiLanguageUtils.AREA_AUTO);
    ...
    public static void saveLanguageSetting(Context context, Locale locale) {
        PreferencesHelper.putStringValue(HealthApplication.getAppViewModel().getApplication(), SP_LANGUAGE, locale.getLanguage());
        PreferencesHelper.putStringValue(HealthApplication.getAppViewModel().getApplication(), SP_COUNTRY, locale.getCountry());
    }
    ...
    public static Application.ActivityLifecycleCallbacks callbacks = new Application.ActivityLifecycleCallbacks() {
        ...
        public void onActivityCreated(Activity activity, Bundle savedInstanceState) {
            String language = PreferencesHelper.getSharedPreferences(HealthApplication.getAppViewModel().getApplication()).getString(SP_LANGUAGE, MultiLanguageUtils.LANGUAGE_AUTO);
            String country = PreferencesHelper.getSharedPreferences(HealthApplication.getAppViewModel().getApplication()).getString(SP_COUNTRY, MultiLanguageUtils.AREA_AUTO);
...
//            Locale locale = getAppLocale(activity);
//            String language = locale.getLanguage();
//            String country = locale.getCountry();
//            Log.e(TAG, "onActivityResumed: "+activity.getClass().getSimpleName() +"language : "+language+ " country : "+country);
            String language = PreferencesHelper.getSharedPreferences(HealthApplication.getAppViewModel().getApplication()).getString(SP_LANGUAGE, MultiLanguageUtils.LANGUAGE_AUTO);
            String country = PreferencesHelper.getSharedPreferences(HealthApplication.getAppViewModel().getApplication()).getString(SP_COUNTRY, MultiLanguageUtils.AREA_AUTO);
            //强制修改应用语言  ，，相对于旧版的是默认也要强行切换，旧版的默认不会去进行切换
            if (!isSameWithSetting(activity)) {//如何区分auto
