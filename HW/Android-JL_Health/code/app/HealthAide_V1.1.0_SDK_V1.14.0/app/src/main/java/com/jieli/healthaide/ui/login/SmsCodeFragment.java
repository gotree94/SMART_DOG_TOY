...
public abstract class SmsCodeFragment extends BaseFragment {
    ...
    public abstract boolean isRegisterUser();
    ...
    private void init() {
        ...
        mViewModel.codeCounterMutableLiveData.observe(getViewLifecycleOwner(), smsCounter -> {
        ...
        if (FormatUtil.checkPhoneNumber(phoneNumber)) {
            ImageCaptchaDialog dialog = new ImageCaptchaDialog(new OnResultCallback<ImageInfo>() {
                ...
                public void onResult(ImageInfo result) {
                    mViewModel.sendSmsCode(isRegisterUser(), phoneNumber, result);
                }
