...
public class ResetEmailPasswordViewModel extends ViewModel {
    ...
    public void checkIdentifyCode(String emailAddress, String code) {
        stateLiveData.postValue(STATE_CHECK_SMS_CODE);
        HttpClient.createUserApi().checkEmailCode(emailAddress, code).enqueue(new Callback<BooleanResponse>() {
            @Override
    ...
    public void resetPassword(String email, String password, String code) {
        ...
        HttpClient.createUserApi().resetPasswordByEmail(email, password, code).enqueue(new Callback<BooleanResponse>() {
            @Override
            public void onResponse(@NonNull Call<BooleanResponse> call, @NonNull Response<BooleanResponse> response) {
                if (response.code() != 200) {
                    onFailure(call, new RuntimeException("http状态码异常"));
