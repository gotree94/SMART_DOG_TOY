...
public class RulerView extends RelativeLayout {
    ...
    private float mScaleValueInterval = 0f;
    private float mPointerHeight = 0;
    private float mPointerWidth = 0;
    ...
    public RulerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mPointerHeight = (float) (density * 32.5);
        mPointerWidth = (float) (density * 12);
        ...
        if (typedArray != null) {
            ...
            mPointerHeight = typedArray.getDimension(R.styleable.fmRulerView_pointerHeight, mPointerHeight);
//            mScaleDistanceInterval = typedArray.getDimension(R.styleable.fmRulerView_intervalDistance, mScaleDistanceInterval);
    ...
    private boolean isRTL() {
        Log.e(TAG, "isRTL: " + (TextUtilsCompat.getLayoutDirectionFromLocale(Locale.getDefault()) == LayoutDirection.RTL));
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mScaleValueInterval = mFMRulerView.getScaleValueInterval();
        ...
        LayoutParams imageViewLayoutParams = new LayoutParams((int) mPointerWidth, (int) mPointerHeight);
...
//        mCurrentPosition = position;
        if (mFMRulerView != null) {
            int tranSection = (int) ((position - mFMRulerView.getMinValue()) / mScaleValueInterval);
            float tranX = mScaleDistanceInterval * tranSection;
            if (isRTL()) {
                int fmRulerViewWidth = (int) ((mFMRulerView.getScaleTotalCount() - 1) * mScaleDistanceInterval + mFMRulerView.getRulerViewWidth());//计算出mFMRulerView的宽度
                tranX = -(fmRulerViewWidth - mFMRulerView.getRulerViewWidth()) + tranX;
            }
            Log.e(TAG, "setCurrentPosition: " + tranX);
    ...
    private class ViewFlinger implements Runnable {
        ...
        public ViewFlinger() {
            mScroller = new OverScroller(mFMRulerView.getContext(), new DecelerateInterpolator(0.2f));
        }
...
//            mIsStartScroll = true;
    ...
    private void constrainScrollBy(int dx, int dy) {
        if (isRTL()) {
            int scrollX = mFMRulerView.getScrollX();//当前滚动
            if (dx + scrollX > 0) {//越过尺的左侧
                dx = -scrollX;
                mViewFlinger.stop();
            } else if (dx - getWidth() + scrollX < -mFMRulerView.getWidth()) {//越过尺的you侧
...
//        JL_Log.d(TAG, "handlerCallback1: value:" + value + "mFMRulerView.getScaleTotalCount():" + mFMRulerView.getScaleTotalCount());
        if (isRTL()) {
            value = mFMRulerView.getScaleTotalCount() - 1 + value;
        }
//        JL_Log.d(TAG, "handlerCallback2: value:" + value + "mFMRulerView.getScaleTotalCount():" + mFMRulerView.getScaleTotalCount());

        if (isFlingEnd && canFlingTrueCallback) {//滑动结束
        ...
        value = (int) (value * mScaleValueInterval);
