...
//    private int mImageScaleResourceId = -1;
    ...
    private boolean mDrawHorizontalLine = false;
    private int mBigScaleIntervalValue = 10;
    /**
     * exist Radio array
     */
    ...
    private void init(AttributeSet attributeSet) {
        ...
        mTextSize = density * 13;
        mScaleDistanceInterval = density * 9;
        mBigScaleLineWidth = (float) (density * 2);
        mSmallScaleLineWidth = density * 2;
        mAllScaleHeight = density * 67;
        mBigScaleMarginTop = density * 0;
        mSmallScaleMarginTop = density * 0;
        ...
        if (typedArray != null) {
            ...
            mDrawHorizontalLine = typedArray.getBoolean(R.styleable.fmRulerView_horizontalLine, mDrawHorizontalLine);
            mBigScaleIntervalValue = typedArray.getInteger(R.styleable.fmRulerView_bigScaleIntervalValue, mBigScaleIntervalValue);
        }
    ...
    private void drawScale(Canvas canvas) {
        for (int i = 0; i < mScaleTotalCount; i++) {
            float offsetX = i * mScaleDistanceInterval + mRulerViewWidth / 2 + mSmallScaleLineWidth / 2;
            int scaleValue = mMinValue + i * mScaleValueInterval;
            if ((mExistRadio != null && mExistRadio.size() > 0) && mExistRadio.contains(scaleValue)) {
                if (Math.abs(scaleValue) % 5 == 0) {
                    ...
                    String text = CalendarUtil.formatString("%.0f", getFloatValue(scaleValue));
                ...
                if (Math.abs(scaleValue) % mBigScaleIntervalValue == 0) {
        ...
        if (mDrawHorizontalLine) {
            canvas.drawLine(mRulerViewWidth / 2 - mSmallScaleLineWidth / 2, mSmallScaleMarginTop + mSmallScaleLineWidth / 2, (mScaleTotalCount - 1) * mScaleDistanceInterval + mRulerViewWidth / 2, mSmallScaleMarginTop + mSmallScaleLineWidth / 2, mSmallScaleLinePaint);
        }
    }
