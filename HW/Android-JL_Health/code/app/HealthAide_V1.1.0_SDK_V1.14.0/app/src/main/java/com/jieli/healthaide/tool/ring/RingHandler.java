package com.jieli.healthaide.tool.ring;
...
public class RingHandler {
    ...
    private RingHandler(Context context) {
        ...
        ringtoneManager = new RingtoneManager(HealthApplication.getAppViewModel().getApplication());
    }
    ...
    public static RingHandler getInstance() {
        if (null == instance) {
            synchronized (RingHandler.class) {
                if (null == instance) {
                    instance = new RingHandler(HealthApplication.getAppViewModel().getApplication());
                }
            }
        }
        return instance;
...
//        Uri currentAlarm = RingtoneManager.getActualDefaultRingtoneUri(mContext, RingtoneManager.TYPE_RINGTONE); //系统当前  来电铃声
        Ringtone ringtone = RingtoneManager.getRingtone(mContext, currentAlarm);
        JL_Log.d(TAG, "getCurrentAlarmRing", "uri : " + currentAlarm + ", ringtone : " + ringtone);
        if (ringtone == null) {
    ...
    public void playAlarmRing(int type, long timeout) {
        ...
        if (mAlarmRing != null) {
            ...
            if (type == RcspConstant.SEARCH_TYPE_DEVICE) { //找设备
