...
public class CombinedChart extends BarLineChartBase<CombinedData> implements CombinedDataProvider {
    ...
    public void setData(CombinedData data) {
        super.setData(data);
        setHighlighter(new CombinedHighlighter(this, this));
        if (isFirstSetData) {//修改，不要一直setData的时候创建新的ChartRenderer,会影响到高亮位置
            isFirstSetData = false;
            ((CombinedChartRenderer) mRenderer).createRenderers();
        }
        mRenderer.initBuffers();
        ((CombinedChartRenderer) mRenderer).notifyDataSetChanged();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ((CombinedChartRenderer) mRenderer).setPointerPosition(this, event);
                break;
    ...
    public float getHighLightXByCurrentPointerPosition() {
        return ((CombinedChartRenderer) mRenderer).getHighLightXByCurrentPointerPosition();
    }
