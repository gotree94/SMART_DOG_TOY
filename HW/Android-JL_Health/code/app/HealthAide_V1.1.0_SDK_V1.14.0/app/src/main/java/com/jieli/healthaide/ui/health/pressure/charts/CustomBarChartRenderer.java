...
package com.jieli.healthaide.ui.health.pressure.charts;

//*****这个CustomBarChart和另一个有一点区别：这个是在游标动的时候，在一定范围内 ，不要立即刷新HighLightX
...
//            Log.d(TAG, "drawValues: trans !isStacked");
        ...
        if (pointerX != DEFAULT_POINTER_X) {//暂时这样优化，第一次的时候不应该使用靠近原则高亮 因为currentPointerX等于0
            //修改:: 高亮判断
            BarEntry highLightE1 = null;
            BarEntry highLightE2 = null;
            BarEntry highLightEntry = null;
            for (int i = 0; i < dataSet.getEntryCount(); i++) {
                BarEntry e = (BarEntry) dataSet.getEntryForIndex(i);
                if (e.getY() == 0f) continue;
                boolean isHighLight = currentPointerX - 27 < e.getX() && e.getX() < currentPointerX + 27;
