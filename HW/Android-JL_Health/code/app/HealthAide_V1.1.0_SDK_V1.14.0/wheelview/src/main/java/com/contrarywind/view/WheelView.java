...
public class WheelView extends View {
    ...
    protected Paint paintOuterText;
    ...
    protected int measuredWidth;// WheelView 控件宽度
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        labelTextSize = (int) (context.getResources().getDisplayMetrics().density * 15);
    ...
    private void initPaints() {
        ...
        paintLabel.setTextSize(context.getResources().getDisplayMetrics().density * 15);
    ...
    public void setLabelTextSize(int size) {
        if (size > 0) {
            labelTextSize = (int) (context.getResources().getDisplayMetrics().density * size);
    ...
    private void drawLabel(Canvas canvas, int measuredWidth, int maxTextWidth, int labelWidth) {
        //todo 修改过label 的位置
        int drawRightContentStart = measuredWidth / 2 + maxTextWidth / 2 + labelWidth / 2;
        drawLabel(canvas,drawRightContentStart);
    }

    protected void drawLabel(Canvas canvas, int drawRightContentStart) {
        //todo 修改过label 的位置
        canvas.drawText(label, drawRightContentStart, centerY, paintLabel);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(label) && isCenterLabel) {
            //绘制文字，靠右并留出空隙
            drawLabel(canvas, measuredWidth, maxTextWidth, getTextWidth(paintLabel, label));
        }
