...
- (instancetype)initWithIndex:(NSInteger)index WithHeight:(float)height
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, 0, index*kMIN_SCALE*kMIN_GAP+kFMPickViewGAP, height);
        view_W = index*kMIN_SCALE*kMIN_GAP+kFMPickViewGAP;
        view_H = height;
...
-(void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx{
    /*--- 描绘刻度 ---*/
    for (NSInteger i = 0; i <= mIndex*kMIN_SCALE; i++)
    {
        /*--- 小刻度 ---*/
        if (i%2 != 0) {
...
-(void)drawScale20:(float)x Context:(CGContextRef)ctx{
    ...
    CGContextMoveToPoint(ctx, x - kMIN_GAP/2.0, 1.0);
    CGContextAddLineToPoint(ctx, x + kMIN_GAP/2.0, 1.0);
