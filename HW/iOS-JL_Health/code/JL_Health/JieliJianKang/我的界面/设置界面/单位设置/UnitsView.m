...
@interface UnitsView()<UITableViewDelegate,UITableViewDataSource>{
    UIView *bgView;
    UIView *view1; //选择单位
    UILabel *genderLabel; //公制
...
-(void)initUI{
    ...
    genderLabel.frame = CGRectMake((sw-32)/2-45/2,20,sw,25);
    genderLabel.numberOfLines = 0;
    genderLabel.text = kJL_TXT("单位");
    ...
    funArray = @[kJL_TXT("公制"),kJL_TXT("英制")];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.hidden = YES;
    if ([_delegate respondsToSelector:@selector(didSelectUnits:)]) {
        [_delegate didSelectUnits:(int)indexPath.row];
    }
