...
-(void)initUI{
    ...
    //命令背景View
    centerView = [[UIView alloc] initWithFrame:CGRectMake(46,[UIScreen mainScreen].bounds.size.height/2-130/2,DeleteView_W-46*2,130)];
    ...
    _titleLab.frame = CGRectMake(19,22,centerView.frame.size.width-38,48);
...
//    }
    ...
    view_1.frame = CGRectMake(0,_titleLab.frame.origin.y+
                              _titleLab.frame.size.height+20,centerView.frame.size.width,0.5);
...
-(void)setType:(int)type{
    ...
    if([kJL_GET hasPrefix:@"zh"]){
        _titleLab.frame = CGRectMake(centerView.frame.size.width/2-220/2,32,220,22);
    }else{
    ...
    if(mType == 0){
        ...
        if([kJL_GET hasPrefix:@"zh"]){
            _titleLab.frame = CGRectMake(centerView.frame.size.width/2-220/2,32,220,44);
        }else{
            _titleLab.frame = CGRectMake(centerView.frame.size.width/2-300/2,32,300,44);
        }
    }else if(mType == 1){
        _titleLab.numberOfLines = 0;
        if([kJL_GET hasPrefix:@"zh"]){
    ...
    _titleLab.textColor = kDF_RGBA(36, 36, 36, 1.0);
    _titleLab.font = [UIFont fontWithName:@"PingFangSC-Medium" size:15];
    [_cancelBtn setTitle:kJL_TXT("取消") forState:UIControlStateNormal];
    [_deleteBtn setTitle:kJL_TXT("删除") forState:UIControlStateNormal];
    // }
    //    if(mType == 1){
...
- (void)cancelBtnAction:(UIButton *)sender {
    ...
    if(mType ==0){
        if ([_delegate respondsToSelector:@selector(didCancelAllAction:)]) {
            [_delegate didCancelAllAction:_cancelBtn];
        }
    }else{
...
- (void)deleteBtnAction:(UIButton *)sender {
    ...
    if(mType ==0){
        if ([_delegate respondsToSelector:@selector(didDeleteAllAction:)]) {
            [_delegate didDeleteAllAction:_deleteBtn];
        }
    }else{
