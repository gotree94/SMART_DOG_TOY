...
- (id)initWithDic:(NSDictionary *)dic{
    ...
    up.email        =   [dict objectForKey:@"email"] ? dict[@"email"]:@"";
...
-(NSDictionary *)beDict{
    NSDateFormatter *fm = [EcTools cachedFm];
    fm.dateFormat = @"yyyy-MM-dd HH:mm:ss";
    NSDictionary *dict = @{@"id":self.identify,
                           @"uuid":self.uuid,
                           @"mobile":self.mobile,
                           @"email":self.email,
                           ...
                           @"status":@(self.status),
                           @"registerTime":[fm stringFromDate:self.registerTime],
                           @"lastLoginTime":[fm stringFromDate:self.lastLoginTime]};
    return dict;
}
...
+ (void)removeProfile{
    [JL_Tools removeUserByKey:userProfile];
}
