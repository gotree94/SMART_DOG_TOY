#ifndef  __EFUSE_H__
#define  __EFUSE_H__

#include "typedef.h"

// bool is_boot_otp();

void efuse_init();
void efuse_dump();

u8 efuse_get_efuse_en_act();
u8 efuse_get_lvd_rst_en();
u8 efuse_get_lvd_act();
u8 efuse_get_flash_reset();
u8 efuse_get_flash_io_sel();
u8 efuse_get_vddio_lvd_lev();
u8 efuse_get_iov_act();
u8 efuse_get_vddio_lev();
u8 efuse_get_mclr_en();
u8 efuse_get_fast_pu_dis();
u8 efuse_get_pinr_reset_en();
// u8 efuse_get_audio_vbg_trim();//no
u8 efuse_get_sfc_fast_boot_dis();
u8 efuse_get_trim_act();
u8 efuse_get_lvd_vbg_lel();
u8 efuse_get_mvbg_lev();
u8 efuse_get_wvbg0_level_trim();
// u8 efuse_get_wvbg1_level_trim();

u32 efuse_get_apa_vb17_trim();
u8 efuse_get_cp_pass();
u8 efuse_get_ft_pass();
u8 efuse_get_wvdd_level();

u32 efuse_get_efuse_page(u8 page);

u32 efuse_get_sys_ldo11a_sel();
// u32 efuse_get_lrc_ds_trim();
// u32 efuse_get_lrc_pll_nr();
u32 efuse_get_lrc_freq_offset();
u32 efuse_get_lrc_freq(void);

u32 efuse_get_current_trim();
u32 efuse_get_ecc_enable();
u32 efuse_get_ecc_code();

u32 efuse_get_key_mode();
u32 efuse_get_chip_id();
u32 get_chip_id();
void get_chip_uuid(u8 *buf);//len:20
u32 get_chip_version();
u32 efuse_get_gpadc_vbg_trim();

u8 pmu_use_softflag();

#define      CHIP_VERSION_A     0x00
#define      CHIP_VERSION_B     0x01






u32 efuse_get_vbg_trim();
u32 efuse_get_hadc_vbg_trim(void);
u32 efuse_get_VBIM_ref_res_trim(void);
u32 efuse_get_wvdd_trim(void);
u32 efuse_get_sfc_fast_boot(void);
u32 efuse_get_chip_key();

#endif  /*EFUSE_H*/

