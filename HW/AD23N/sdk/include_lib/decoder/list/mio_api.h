...
struct mio_info {
    ...
    u8 remain;
    u16 rate;
    u8 pwm_total;   //对应mio文件的pwm通道个数
    u8 io_total;    //对应mio文件的IO通道个数
    u8 remain1[2];
};
...
#define d_mio_module_init() if(HAS_MIO_EN) { mio_module_init();}
#define d_mio_start(n)      if(HAS_MIO_EN) { mio_start(n);}
#define d_mio_kick(n,m)     if(HAS_MIO_EN) { mio_kick(n,m);}
#define d_mio_run()         if(HAS_MIO_EN) { mio_run();}
#define d_mio_open(n,m,k)   if(HAS_MIO_EN) { mio_open(n,m,k);}
#define d_mio_close(n)      if(HAS_MIO_EN) { mio_close(n);}
