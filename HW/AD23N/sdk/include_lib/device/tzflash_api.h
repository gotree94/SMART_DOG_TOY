...
struct jlfs_image_head_t {
    const struct jlfs_file_head *flash_head;
    const struct jlfs_file_head *uboot;
    const struct jlfs_file_head *isd_config;
    const struct jlfs_file_head *votp_cfg;
    const struct jlfs_file_head *app_dir_head;
    const struct jlfs_file_head *app_a_head; //app.bin
    const struct jlfs_file_head *app_dir_head2;
    const struct jlfs_file_head *app_b_head; //app.bin
    const void *uboot_entry;
    const void *ini_entry;
    const void *app_a_entry;
    const void *app_b_entry;
    u32 app_mmu_entry;
    u32 uboot_mmu_entry;
    u32 base_addr;
};

extern struct jlfs_image_head_t jlfs_image_head;
#endif
