...
void LED5X7_clear(void)
{
    ...
    for (u8 i = 0; i < 16; i++) {
        if (LED_PORT_ALL & BIT(i)) {
            JL_PORTA->PU  &= ~(BIT(2 * i) | BIT(2 * i + 1));
            JL_PORTA->PD  &= ~(BIT(2 * i) | BIT(2 * i + 1));
        }
    }
    JL_PORTA->HD &= ~LED_PORT_ALL;
}
...
void LED5X7_show_music_main(int arg)
{
    ...
    if (if_decoder_is_run(tmp_dec_pctl->p_dec_obj)) {
        LED_STATUS |= LED_PLAY;
    } else {
...
void LED5X7_scan(void)
{
    ...
    JL_PORTA->DIR &= ~tmp;
    /* tmp = led_used_io[cnt]; */
    /* gpio_set_mode(IO_PORT_SPILT(tmp), PORT_OUTPUT_HIGH); */
