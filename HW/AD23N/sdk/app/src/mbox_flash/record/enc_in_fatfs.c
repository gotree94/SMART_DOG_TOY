...
int fatfs_enc_file_create(Encode_Control *obj)
{
    ...
    err = vfs_mount(&obj->pfs, obj->device, "fat");
    if (err) {
    ...
    vfs_get_encfolder_info(obj->pfs, (char *)REC_FOLDER_PATH, "MP3", &last_num, &total_rec_file);
    if (total_rec_file == 0) {
        vfs_mk_dir(obj->pfs, (char *)REC_FOLDER_PATH, 0);
    }
    /* last_num++; */
...
__fat_rec_file_name:
    ...
    err = vfs_openbypath(obj->pfs, &obj->pfile, path_buff);
    if (0 == err) {
        log_info("file %d exist!\n", last_num);
        ...
        void *path = path_buff;
        err = vfs_createfile(obj->pfs, &obj->pfile, (u32 *)&path[0]);
        if (err) {
...
dec_obj *fatfs_enc_file_decode(Encode_Control *obj, u16 dec_type)
{
    ...
    err = vfs_openbypath(obj->pfs, &obj->pfile, (const char *)obj->rec_file_path);
    if (err) {
