...
void spi_auto_test(hw_spi_dev spi_master, hw_spi_dev spi_slave)
{
    ...
    struct spi_platform_data spix_p_data_test_s = {
        .port = {
            IO_PORTA_06, //clk any io
            IO_PORTA_07, //do any io
...
//init 中断模式
    int ret = spi_open(spi_master, &spix_p_data_test_m);
    if (ret < 0) {
        log_error("spi master init error(%d)!", ret);
        while (1) {
            wdt_clear();
    ...
    ret = spi_open(spi_slave, &spix_p_data_test_s);
    if (ret < 0) {
        log_error("spi slave init error(%d)!", ret);
        while (1) {
            wdt_clear();
...
//dma tx rx(中断)
    ...
    while (ii--) {
        ...
        log_info_hexdump(spi_tx_buf_test, _tx_len);
        ...
        log_info("spi(%d) master irq dma rx ok!len:%d\n", spi_master, hw_spix_get_isr_len(spi_master));
        log_info_hexdump(spi_rx_buf_test, sizeof(spi_rx_buf_test));
        if (memcmp(spi_rx_buf_test, spi_tx_buf_test, _tx_len) == 0) { //ok
            log_info("spi(%d) master <<>>spi(%d) slave irq dma ok!\n", spi_master, spi_slave);
        } else { //error
            log_error("spi(%d) master <<>>spi(%d) slave irq dma error!\n", spi_master, spi_slave);
            while (1) {
                wdt_clear();
            }
        }
...
//buf 接口
        ...
        log_info_hexdump(spi_s_rx_buf_test, sizeof(spi_s_rx_buf_test));
        if (memcmp(spi_s_rx_buf_test, spi_tx_buf_test, _tx_len) == 0) { //ok
            log_info("spi(%d) master >> spi(%d) slave irq byte tx&rx ok!\n", spi_master, spi_slave);
        } else { //error
            log_info_hexdump(spi_tx_buf_test, _tx_len);
            log_error("spi(%d) master >> spi(%d) slave irq byte tx&rx error!\n", spi_master, spi_slave);
            while (1) {
                wdt_clear();
            }
        }

        if (memcmp(spi_rx_buf_test, spi_s_tx_buf_test, _tx_len) == 0) { //ok
            log_info("spi(%d) master << spi(%d) slave irq byte tx&rx ok!\n", spi_master, spi_slave);
        } else { //error
            log_info_hexdump(spi_s_tx_buf_test, _tx_len);
            log_error("spi(%d) master << spi(%d) slave irq byte tx&rx error!\n", spi_master, spi_slave);
            while (1) {
                wdt_clear();
            }
    ...
    log_info("spi(%d) master spi(%d) slave irq dma/byte tx&rx success!\n", spi_master, spi_slave);
    while (1) {
        wdt_clear();
        mdelay(1000);
    }
