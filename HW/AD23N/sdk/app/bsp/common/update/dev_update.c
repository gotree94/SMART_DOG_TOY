...
int boot_info_res_deal(int ret)
{
    if (ret == 0) {
        y_printf(">>>[test]:reset!!!!!!!!\n");
        chip_reset();
...
void dual_update_by_sd_fs()
{
    /* y_printf(">>>[test]:----------------------test return!!!!!!!!!!!\n"); */
    /* return; */
    ...
    y_printf(">>>[test]:-----fopen db_update_data.bin\n");
    vfs_openbypath(pfs, &pfile, "/db_up.bin");
    if (!pfile) {
        //没有升级文件， 继续跑其他解码相关的流程
    ...
    //TODO:winhex拿到升级文件具体地址和大小
    /* u32 fw_crc = 0xDF29; */
    u32 fw_size = 0;
    vfs_get_fsize(pfile, &fw_size);
    u32 max_pkt_len = 512;
    int ret;
    /* ret = dual_bank_passive_update_init(fw_crc, fw_size, max_pkt_len, NULL); */
    ret = dual_bank_passive_update_init(0, fw_size, max_pkt_len, NULL);
    y_printf("\n >>>[test]:func = %s,line= %d, ret = %d\n", __FUNCTION__, __LINE__, ret);
    ret = dual_bank_update_allow_check(fw_size);
    y_printf("\n >>>[test]:func = %s,line= %d, ret = %d\n", __FUNCTION__, __LINE__, ret);
    //sd读，然后写
    u8 *wbuf = malloc(max_pkt_len);
    /////////////////

    while (1) {
        u32 rlen = vfs_read(pfile, wbuf, max_pkt_len);
        /* y_printf(">>>[test]:rlen = %d\n", rlen); */
        /* put_buf(wbuf, rlen); */
        if (rlen <= 0) {
            break;
        }
        ret = dual_bank_update_write(wbuf, rlen, NULL);
    }
    dev_close(device);
    /////////////////
    /* dual_bank_update_verify(NULL, NULL, NULL); */
    dual_bank_update_verify_without_crc();
    dual_bank_update_burn_boot_info(boot_info_res_deal);
