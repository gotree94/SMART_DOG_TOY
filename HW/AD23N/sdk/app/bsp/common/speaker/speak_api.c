...
typedef struct __audio_adc_speaker {
    ...
    EFFECT_OBJ    *p_notchhowling;            // p_curr_sound->effect;
    EFFECT_OBJ    *p_pitchshifthowling;            // p_curr_sound->effect;
    EFFECT_OBJ    *p_echo;            // p_curr_sound->effect;
    EFFECT_OBJ    *p_voicepitch;            // p_curr_sound->effect;
    EFFECT_OBJ    *p_voicechanger;            // p_curr_sound->effect;
...
void audio_adc_speaker_start(void)
{
...
#if defined(NOTCH_HOWLING_EN) && (NOTCH_HOWLING_EN) //陷波抑制啸叫
    /* p_curr_sound = link_notch_howling_sound(p_curr_sound, &cbuf_ads_o, (void **)&aa_speaker.p_notchhowling, adc_sr); */
#endif
#if defined(PITCHSHIFT_HOWLING_EN) && (PITCHSHIFT_HOWLING_EN) //移频抑制啸叫
    /* p_curr_sound = link_pitchshift_howling_sound(p_curr_sound, &cbuf_ads_o, (void **)&aa_speaker.p_pitchshifthowling, adc_sr); */
#endif
    ...
    /* p_curr_sound = link_voice_pitch_sound(p_curr_sound, &cbuf_ads_o, (void **)&aa_speaker.p_voicepitch, VP_CMD_ROBOT); */
#endif
#if defined(VO_CHANGER_EN) && (VO_CHANGER_EN)
    /* p_curr_sound = link_voice_changer_sound(p_curr_sound, &cbuf_ads_o, (void **)&aa_speaker.p_voicechanger, adc_sr); */
#endif
#if defined(PCM_EQ_EN) && PCM_EQ_EN
    /* p_curr_sound = link_pcm_eq_sound(p_curr_sound, &cbuf_ads_o, (void **)&aa_speaker.p_eq, adc_sr, 1); */
#endif
...
void audio_adc_speaker_reless(void)
{
    if (aa_speaker.sound.enable & B_DEC_RUN_EN) {
        ...
        if (NULL != aa_speaker.p_eq) {
            eq_release((void **)&aa_speaker.p_eq);
        }
#endif
#if defined(ECHO_EN) && (ECHO_EN) //混响
        if (NULL != aa_speaker.p_echo) {
            echo_release((void **)&aa_speaker.p_echo);
        }
#endif

#if defined(VO_PITCH_EN) && (VO_PITCH_EN) //多种音效集合库：机器人、rap、两种变速变调；
        if (NULL != aa_speaker.p_voicepitch) {
            voicepitch_release((void **)&aa_speaker.p_voicepitch);
        }
#endif
#if defined(VO_CHANGER_EN) && (VO_CHANGER_EN)
        if (NULL != aa_speaker.p_voicechanger) {
            voicechanger_release((void **)&aa_speaker.p_voicechanger);
        }
#endif
#if defined(PITCHSHIFT_HOWLING_EN) && (PITCHSHIFT_HOWLING_EN) //移频抑制啸叫
        if (NULL != aa_speaker.p_pitchshifthowling) {
            pitchshift_howling_release((void **)&aa_speaker.p_pitchshifthowling);
        }
#endif
#if defined(NOTCH_HOWLING_EN) && (NOTCH_HOWLING_EN) //陷波抑制啸叫
        if (NULL != aa_speaker.p_notchhowling) {
            notchhowling_release((void **)&aa_speaker.p_notchhowling);
        }
#endif
        memset(&aa_speaker, 0, sizeof(aa_speaker));
    }
