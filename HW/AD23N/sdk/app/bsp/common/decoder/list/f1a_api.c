...
#define D_F1A_DEC_RESOURCE (sizeof(f1x_data))
...
#define D_THIS         f1a_dec
#define D_THIS_NAME    "f1a_dec"
#define D_THIS_DEC_RESOURCE   D_F1A_DEC_RESOURCE
static u8 EXPAND_CONCAT(D_THIS, _mcnt) = 0;
D_DUMP_CNT();

u32 f1a_decoder_res_release(void *priv)
{
    dec_obj *obj = priv;
    if (obj == NULL) {
        return EINVAL;
...
u32 f1a_decode_index(void *p_file, u32 index, dec_obj **p_dec, void *p_dp_buf)
{
    u32 buff_len, i;
    decoder_ops_t *ops;
    /* void *name; */
    ...
    if (p_dec_hld->pdec_res != NULL) {
        //上次没释放
        log_error("dec_res_not_null 0x%x\n", p_dec_hld->pdec_res);
        f1a_decoder_res_release(p_dec_hld);
    }
    local_irq_disable();
    memset(p_dec_hld, 0, sizeof(dec_obj));
    local_irq_enable();

    D_MALLOC(p_dec_hld->pdec_res,  D_THIS_DEC_RESOURCE);
    log_info("malloc_pf1x_data 0x%x\n", p_dec_hld->pdec_res);
    if (NULL == p_dec_hld->pdec_res) {
        log_error("f1a_cannot_malloc_any_resource\n");
        return E_F1A_RESOURCE;
    }

    f1x_data *p_f1x_data = p_dec_hld->pdec_res;
    ...
    p_dec_hld->decoder_res_release = f1a_decoder_res_release;
    /* log_info("C\n"); */
    //
    ...
    if (check_ext_api(g_file_sname, f1a_ext, 4)) {
        ...
        ctype = 'x';
        void *pf1x = f1x_play_api(p_file, g_loop_tab, sizeof(g_loop_tab), index, t_par, &p_f1x_data->f1x_ctl_hdl);
        if (NULL == pf1x) {
            log_error("f1x null\n");
            f1a_decoder_res_release(p_dec_hld);
            return E_F1X_FORMAT;
...
u32 f1a_decode_api_1(void *p_file, void **p_pdec, void *t_dp_buf)
{
    ...
    return f1a_decode_index(p_file, 0, (dec_obj **)p_pdec, t_dp_buf);
}
...
u32 f1a_decode_api_2(void *p_file, void **p_pdec, void *t_dp_buf)
{
    ...
    return f1a_decode_index(p_file, 1, (dec_obj **)p_pdec, t_dp_buf);
#else
    return E_F1A_OUTRGE;
#endif
}
