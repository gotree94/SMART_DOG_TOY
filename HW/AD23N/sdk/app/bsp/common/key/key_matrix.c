...
static void matrix_key_io_set(u8 scan_ptr)
{
    ...
    for (int i = 0; i < ARRAY_MEMS(y_port); i++) {
        if (i == scan_ptr) {
            ///输出1
            gpio_hw_set_direction(IO_PORT_SPILT(y_port[i]), 0);
            gpio_hw_set_pull_down(IO_PORT_SPILT(y_port[i]), 0);
            gpio_hw_set_pull_up(IO_PORT_SPILT(y_port[i]), 0);
            gpio_hw_write(y_port[i], 1);
            continue;
        }

        ///输入高阻
        gpio_hw_set_die(IO_PORT_SPILT(y_port[i]), 0);
        gpio_hw_set_direction(IO_PORT_SPILT(y_port[i]), 1);
        gpio_hw_set_pull_down(IO_PORT_SPILT(y_port[i]), 0);
        gpio_hw_set_pull_up(IO_PORT_SPILT(y_port[i]), 0);
        gpio_hw_write(y_port[i], 0);
    }
}
...
static void matrix_key_adc_cb(void)
{
    ...
    for (int i = 0; i < ARRAY_MEMS(x_adc_ch); i++) {
        u16 adc_value = adc_get_value(adc_io2ch(x_adc_ch[i]));
...
void matrix_key_init(void)
{
    ...
    ///x 轴io init
    for (int i = 0; i < ARRAY_MEMS(x_adc_ch); i++) {
        gpio_hw_set_die(IO_PORT_SPILT(x_adc_ch[i]), 0);
        gpio_hw_set_direction(IO_PORT_SPILT(x_adc_ch[i]), 1);
        gpio_hw_set_pull_down(IO_PORT_SPILT(x_adc_ch[i]), 1);
        gpio_hw_set_pull_up(IO_PORT_SPILT(x_adc_ch[i]), 0);

        adc_add_sample_ch(adc_io2ch(x_adc_ch[i]));
    }

    adc_scan_over_callback_regist(matrix_key_adc_cb);
