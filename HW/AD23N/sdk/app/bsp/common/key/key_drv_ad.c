...
void ad_key_init(void)
{
    ...
    gpio_hw_set_die(IO_PORT_SPILT(AD_KEY_CH), 0);
    gpio_hw_set_direction(IO_PORT_SPILT(AD_KEY_CH), 1);
    gpio_hw_set_pull_down(IO_PORT_SPILT(AD_KEY_CH), 0);
#if EXTERN_R_UP
    gpio_hw_set_pull_up(IO_PORT_SPILT(AD_KEY_CH), 0);
#else
    gpio_hw_set_pull_up(IO_PORT_SPILT(AD_KEY_CH), 1);
#endif

    adc_add_sample_ch(adc_io2ch(AD_KEY_CH));
}
...
u32 get_adkey_value(void)
{
    u8 key_number;
    u32 key_value;
    static u8 key_wrong_cnt = 0;
    key_value = adc_get_value(adc_io2ch(AD_KEY_CH));
    ad_ir_reuse();
    if (sadc_value_wrong(key_value)) {
