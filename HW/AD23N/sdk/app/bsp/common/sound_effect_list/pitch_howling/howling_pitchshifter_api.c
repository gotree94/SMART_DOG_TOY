...
static void *pitchshift_howling_api(void *obuf, u32 sr, void **ppsound)
{
    ...
    if (HOWLING_WORK_BUF_SIZE < buf_len) {
        log_error("howing work buf less %d, need len %d", HOWLING_WORK_BUF_SIZE, buf_len);
        return NULL;
    }

    return howling_phy(obuf, &phparm, sr, ppsound);
}
...
{
    update_howling_parm_fs(peffect, sr, new_fs);
}
#endif
