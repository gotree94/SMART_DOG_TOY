...
#define VOICE_PITCH_WORKBUF_SIZE  (10304)

typedef struct __VOICEPITCH_HDL {
    EFFECT_OBJ vp_obj;
    sound_in_obj vp_si;
    VP_IO_CONTEXT vp_pitch_io;
    VOICE_PITCH_PARA_STRUCT vp_parm;
    u8 VP_BUFLEN[VOICE_PITCH_WORKBUF_SIZE];
} VOICEPITCH_HDL;

#define VP_MAX_MALLOC_CNT  20
static u8 s_voicepitch_hdl_mcnt = 0;

static void dump_voicepitch_hdl_mcnt()
{
    log_info("s_voicepitch_hdl_mcnt %d\n", s_voicepitch_hdl_mcnt);
}
const struct mcnt_operations voicepitch_hdl_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_voicepitch_hdl_mcnt,
};

void voicepitch_release(void **ppeffect)
{
    VOICEPITCH_HDL *p_vp_data = *ppeffect;
    if (NULL != p_vp_data) {
        log_info("FREE VP_DATA:0x%x\n", (u32)p_vp_data);
        /* 再释放音效_data */
        D_MALLOC_CNT_DEC(s_voicepitch_hdl_mcnt);
        free(p_vp_data);
        *ppeffect = NULL;
    }
}
...
void rap_reopen(void **ppeffect)
{
    ...
    ops->open((void *)&p_vp_data->VP_BUFLEN[0], &p_vc_parm, NULL);
}
...
void *vp_phy(void *obuf, VOICE_PITCH_PARA_STRUCT *pvp_parm, void **ppsound)
{
    ...
    if (buff_len > VOICE_PITCH_WORKBUF_SIZE) {
        log_error("vo pitch buff is not enough big! need %d\n", buff_len);
        return NULL;
    }
    D_MALLOC_CNT_INC(s_voicepitch_hdl_mcnt, "s_voicepitch_hdl_mcnt", VP_MAX_MALLOC_CNT);
    VOICEPITCH_HDL *p_vp_hdl = malloc(sizeof(VOICEPITCH_HDL));
    log_info("MALLOC p_vp_hdl 0x%x\n", p_vp_hdl);
    if (NULL == p_vp_hdl) {
        log_error("voicepitch_cannot_malloc_any_ram!!\n");
        return NULL;
    }
    log_info("o1 sound 0x%x\n", p_vp_hdl->vp_obj.sound);
    vp_obj = &p_vp_hdl->vp_obj;
    p_vp_hdl->vp_pitch_io.priv   = &vp_obj->sound;
    p_vp_hdl->vp_pitch_io.output = sound_output;
    memcpy(&p_vp_hdl->vp_parm, pvp_parm, sizeof(VOICE_PITCH_PARA_STRUCT));
    /******************************************/
    //初始化：rs_buf：运算Buf; rs_parm：参数指针，传完可以释放的，里面不会记录这个指针的。vp_pitch_io:output接口，说明如下
    ops->open((void *)&p_vp_hdl->VP_BUFLEN[0], pvp_parm, (void *)&p_vp_hdl->vp_pitch_io);
    /*************************************************/
    memset(&p_vp_hdl->vp_obj, 0, sizeof(EFFECT_OBJ));
    p_vp_hdl->vp_si.ops = ops;
    p_vp_hdl->vp_si.p_dbuf = &p_vp_hdl->VP_BUFLEN[0];
    /*************************************************/
    vp_obj->p_si = &p_vp_hdl->vp_si;
    vp_obj->run = vp_run;
    vp_obj->sound.p_obuf = obuf;
    *ppsound = &vp_obj->sound;

    log_info("p_vp_hdl->vp_obj.sound.p_obuf 0x%x\n", vp_obj->sound.p_obuf);
    log_info("o2 sound 0x%x\n", &vp_obj->sound);
    log_info("vp_succ\n");
    return &p_vp_hdl->vp_obj;
}
