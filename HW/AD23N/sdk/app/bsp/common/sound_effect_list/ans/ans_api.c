...
#define READSIZE                (256)     //每次run处理样点数
#define ANS_RUN_BUFFSIZE        (12664)
#define ANS_TMP_BUFSIZE         (6168)
#define ANS_OUTBUF_SIZE         (READSIZE * 2)
...
typedef struct _ANS_HDL {
    EFFECT_OBJ ans_effect_obj;//必须在第一个
    sound_in_obj ans_si;
    remain_ops ans_remain_ops;
    void *ans_output_buff[ANS_OUTBUF_SIZE];
} ANS_HDL;
/* --------------------------------- */

#define ANS_MAX_MALLOC_CNT  20
static u8 s_ans_hdl_mcnt = 0;

static void dump_ans_hdl_mcnt()
{
    log_info("s_ans_hdl_mcnt %d\n", s_ans_hdl_mcnt);
}
const struct mcnt_operations ans_hdl_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_ans_hdl_mcnt,
};

void ans_release(void **ppeffect)
{
    ANS_HDL *p_ans_data = *ppeffect;
    if (NULL != p_ans_data) {
        log_info("FREE1 ANS_DATA:0x%x\n", (u32)p_ans_data);
        /* 再释放音效_data */
        D_MALLOC_CNT_DEC(s_ans_hdl_mcnt);
        free(p_ans_data);
        *ppeffect = NULL;
    }
}
...
{
    /* log_info("hld 0x%x, inbuf 0x%x\n", hld, inbuf); */
    u32 rlen = 0;
    sound_in_obj *p_si = hld;
    ANS_HDL *ans_hdl = p_si->priv;
    remain_ops *p_ans_remain_ops = &ans_hdl->ans_remain_ops;
    EFFECT_OBJ *p_ans_effect_obj = &ans_hdl->ans_effect_obj;

    /* 0. output 上次剩余的数据 */
    remain_output(&p_ans_effect_obj->sound, p_ans_remain_ops);
    ...
    /* 1. 新一轮输入输出 */
    memset(p_ans_remain_ops->obuf, 0, ANS_OUTBUF_SIZE);    //清空outdata
    ...
    /* 3. 运算run */
    NoiseSuppress_Process(ans_runbuf, ans_tmpbuf, inbuf, p_ans_remain_ops->obuf, NULL, NULL, READSIZE);
...
{
    D_MALLOC_CNT_INC(s_ans_hdl_mcnt, "s_ans_hdl_mcnt", ANS_MAX_MALLOC_CNT);
    ANS_HDL *ans_hdl = malloc(sizeof(ANS_HDL));
    if (NULL == ans_hdl) {
        log_error("ans_cannot_malloc_any_ram\n");
        return NULL;
    }
    memset(&ans_hdl->ans_effect_obj, 0, sizeof(EFFECT_OBJ));

    /* 配给前级 */
    ans_hdl->ans_effect_obj.p_si = &ans_hdl->ans_si;
    ans_hdl->ans_effect_obj.run = (void *)ans_run;

    ans_hdl->ans_effect_obj.sound.p_obuf = obuf;
    *ppsound = &ans_hdl->ans_effect_obj.sound;

    u32 ret = init_remain_obuf(&ans_hdl->ans_remain_ops, sound_output, ans_hdl->ans_output_buff, ANS_OUTBUF_SIZE);
    if (ret) {
    ...
    ans_hdl->ans_si.priv = ans_hdl;
    return &ans_hdl->ans_effect_obj;
...
void *ans_api(void *obuf, void **ppsound, u32 sr)
{
    ...
    log_info("tolbufsize %d %d\n", tolbufsize, ANS_RUN_BUFFSIZE);
    /* ASSERT(ANS_RUN_BUFFSIZE >= tolbufsize); */
    if (ANS_RUN_BUFFSIZE < tolbufsize) {
        return NULL;
    }

    int maxtmpbufsize = NoiseSuppress_QueryTempBufSize(NS_MODE, NS_IS_WIDEBAND);
    log_info("maxtmpbufsize %d %d\n", maxtmpbufsize, ANS_TMP_BUFSIZE);
    /* ASSERT(ANS_TMP_BUFSIZE >= maxtmpbufsize); */
    if (ANS_TMP_BUFSIZE < maxtmpbufsize) {
        return NULL;
    }

    float ANS_AggressFactor = 1.25f;/*范围：1~2,动态调整,越大越强(1.25f)*/
    float ANS_MinSuppress = 0.04f;	/*范围：0~1,静态定死最小调整,越小越强(0.1f)*/
    float ANS_NoiseLevel = 1e2f;      /*范围：-100dB ~ -40dB (-75dB) (1429 = (10^(-75/20))*2^23)*/
...
*/
/*----------------------------------------------------------------------------*/
void *link_ans_sound(void *p_sound_out, void *p_ans_obuf, void **pp_effect, u32 sr)
{
    ...
    if (NULL != p_curr_sound->effect) {
        ...
        p_curr_sound->p_obuf = p_ans_obuf;
