...
static u8 g_fatfs_fs_malloc_cnt = 0;
FATFS *fat_fshdl_alloc(void)
{
    D_MALLOC_CNT_INC(g_fatfs_fs_malloc_cnt, __FUNCTION__, 255);
...
{
    D_MALLOC_CNT_DEC(g_fatfs_fs_malloc_cnt);
    return my_free(fshdl);
}

static void dump_fatfs_mcnt()
{
    log_info("g_fatfs_fs_malloc_cnt %d\n", g_fatfs_fs_malloc_cnt);
}

const struct mcnt_operations fat_fs_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_fatfs_mcnt,
};


static u8 g_fatfs_file_malloc_cnt = 0;
FIL *fat_fhdl_alloc(void)
{
    D_MALLOC_CNT_INC(g_fatfs_file_malloc_cnt, __FUNCTION__, 255);
...
{
    D_MALLOC_CNT_DEC(g_fatfs_file_malloc_cnt);
    return my_free(pfile);
}

static void dump_fatfile_mcnt()
{
    log_info("g_fatfs_file_malloc_cnt %d\n", g_fatfs_file_malloc_cnt);
}

const struct mcnt_operations fat_file_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_fatfile_mcnt,
};
...
static u8 g_fatfs_wbuf_malloc_cnt = 0;
SWIN_BUF *fat_wbuf_alloc(void)
{
    D_MALLOC_CNT_INC(g_fatfs_wbuf_malloc_cnt, __FUNCTION__, 255);
    return  my_malloc(sizeof(SWIN_BUF), MM_SWIN_BUF);
}

SWIN_BUF *fat_wbuf_free(SWIN_BUF *tmp_wbuf)
{
    D_MALLOC_CNT_DEC(g_fatfs_wbuf_malloc_cnt);
    return my_free(tmp_wbuf);

}
static void dump_fatwbuf_mcnt()
{
    log_info("g_fatfs_wbuf_malloc_cnt %d\n", g_fatfs_wbuf_malloc_cnt);
}
const struct mcnt_operations fat_wbuf_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_fatwbuf_mcnt,
};

static u8 g_fatfs_scan_malloc_cnt = 0;
VFSCAN *fat_vfscan_alloc(void)
{
    D_MALLOC_CNT_INC(g_fatfs_scan_malloc_cnt, __FUNCTION__, 255);
    return  my_malloc(sizeof(VFSCAN), MM_VFSCAN_BUF);
}

VFSCAN *fat_vfscan_free(VFSCAN *fsn_wbuf)
{
    D_MALLOC_CNT_DEC(g_fatfs_scan_malloc_cnt);
    return my_free(fsn_wbuf);

}
static void dump_fatscan_mcnt()
{
    log_info("g_fatfs_scan_malloc_cnt %d\n", g_fatfs_scan_malloc_cnt);
}
const struct mcnt_operations fat_scan_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_fatscan_mcnt,
};
...
int fat_ioctl_api(void *pfile, int cmd, int arg)
{
    ...
    case FS_IOCTL_FILE_ATTR: {
        struct vfs_attr *pfile_attr = (struct vfs_attr *)arg;
        fat_attrs_api(pfile, (void *)pfile_attr);
    }
    return 0 ;
    default:
