...
static u8 g_sydfs_file_malloc_cnt = 0;
SYDFILE *sydf_fhdl_malloc(void)
{
    D_MALLOC_CNT_INC(g_sydfs_file_malloc_cnt, __FUNCTION__, 255);
    return  my_malloc(sizeof(SYDFILE), MM_SYDFF);
}

SYDFILE *sydf_fhdl_free(SYDFILE *pfile)
{
    D_MALLOC_CNT_DEC(g_sydfs_file_malloc_cnt);
    return my_free(pfile);
}
static void dump_sydf_file_mcnt()
{
    log_info("g_sydfs_file_malloc_cnt %d\n", g_sydfs_file_malloc_cnt);
}
const struct mcnt_operations free_fs_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_sydf_file_mcnt,
};

static u8 g_sydfs_fs_malloc_cnt = 0;
SYDFS *sydf_fshdl_malloc(void)
{
    D_MALLOC_CNT_INC(g_sydfs_fs_malloc_cnt, __FUNCTION__, 255);
    return my_malloc(sizeof(SYDFS), MM_SYDFS);
}

SYDFS *sydf_fshdl_free(SYDFS *pfs)
{
    D_MALLOC_CNT_DEC(g_sydfs_fs_malloc_cnt);
    return my_free(pfs);
}
static void dump_sydf_fs_mcnt()
{
    log_info("g_sydfs_fs_malloc_cnt %d\n", g_sydfs_fs_malloc_cnt);
}
const struct mcnt_operations sydf_fs_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_sydf_fs_mcnt,
};
#endif
...
u32 sydf_openbysclust_api(void *pfs, void **ppfile, u32 sclust)
{
