...
void norfs_init_api(void)
{
    ...
    struct vfs_attr *p_vm_attr = get_vm_attr_p();
    sector_size = (1 << sector_bit);
    sector_start = (p_vm_attr->sclust + sector_size - 1) / sector_size;
    sector_end = (p_vm_attr->fsize + p_vm_attr->sclust) / sector_size;

    sector_end--;
    log_info("sector_start 0x%x, sector_size 0x%x, sector_end 0x%x\n", sector_start, sector_size, sector_end);
