...
static u8 g_vfs_fhdl_malloc_cnt = 0;
struct imount *vfs_hdl_malloc(void)
{
    D_MALLOC_CNT_INC(g_vfs_fhdl_malloc_cnt, __FUNCTION__, 255);
    return my_malloc(sizeof(struct imount), MM_VFS);
}

struct imount *vfs_fhdl_free(struct imount *phdl)
{
    D_MALLOC_CNT_DEC(g_vfs_fhdl_malloc_cnt);
    return my_free(phdl);
}

static void dump_vfs_fs_mcnt()
{
    log_info("g_vfs_fhdl_malloc_cnt %d\n", g_vfs_fhdl_malloc_cnt);
}

const struct mcnt_operations vfs_fs_mcnt sec_used(.d_malloc_cnt) = {
    .malloc_cnt_dump = dump_vfs_fs_mcnt,
};
#endif
