...
void usb_host_test()
{
    usb_host_mount(0, 3, 20, 200);
    struct device *device;
    int ret = usb_stor_open("udisk0", &device, NULL);
    /* device = dev_open("udisk", NULL); */
    if (ret) {
        log_error("udisk open err!! %s---%d\n", __func__, __LINE__);
        return;
    }
    u32 lba = 0;
    while (1) {
