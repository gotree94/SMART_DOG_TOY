...
int norflash_original_read(u8 *buf, u32 offset, u32 len);
int norflash_original_write(u8 *buf, u32 offset, u32 len);
int norflash_original_erase(u32 cmd, u32 addr);
...
void dual_bank_test()
{
    ...
    for (u32 i = 0; i < (file_size / flash_alignsize); i ++) {
        erase_err = norflash_original_erase(flash_erase_cmd, upgrade_start_addr + i * flash_alignsize);
        if (erase_err) {
    ...
    while (file_size) {
        ...
        norflash_original_write(tmp_buf, upgrade_start_addr + offset, cnt);
    ...
    while (file_size) {
        ...
        vfs_read(pfile, tmp_buf, cnt);
        norflash_original_read(f_tmp_buf, upgrade_start_addr + offset, cnt);
        /* log_info("\n \n"); */
        /* log_info("file data"); */
...
__verify_fail:
    norflash_original_erase(IOCTL_ERASE_SECTOR, upgrade_start_addr);
    my_free((void *)f_tmp_buf);
    my_free((void *)tmp_buf);
__write_end:
__close_file:
