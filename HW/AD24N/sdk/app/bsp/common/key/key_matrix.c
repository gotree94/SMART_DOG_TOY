...
void matrix_key_init(void)
{
    ...
    ///x 轴io init
    for (int i = 0; i < ARRAY_MEMS(x_adc_ch); i++) {
        gpio_hw_set_die(IO_PORT_SPILT(adc_ch2port(x_adc_ch[i])), 0);
        gpio_hw_set_direction(IO_PORT_SPILT(adc_ch2port(x_adc_ch[i])), 1);
        gpio_hw_set_pull_down(IO_PORT_SPILT(adc_ch2port(x_adc_ch[i])), 1);
        gpio_hw_set_pull_up(IO_PORT_SPILT(adc_ch2port(x_adc_ch[i])), 0);
