...
void toy_music_app(void)
{
    ...
    /* FIX文件播放msg start */
    /* 如果需要演示播放f1x文件，可以直接打开下面注释的post_msg代码, 同时需要注释上面的post_msg(1, MSG_PLAY_FILE1) */
    /* post_msg(1, MSG_PLAY_F1X_DEMO); */
    /* FIX文件播放msg end */
    ...
    while (1) {
        ...
        /* FIX文件播放demo start */
        /* 该case为公版SDK直接播放f1x文件的demo, 如果资源文件有变化，需要修改dec_pctl[0].pdir和dec_pctl[0].findex*/
        case MSG_PLAY_F1X_DEMO:
            decoder_stop(dec_pctl[0].p_dec_obj, NEED_WAIT);
            log_info("MSG_PLAY_F1X_DEMO\n");
            dec_pctl[0].findex      = 1;
            dec_pctl[0].pdir        = (void *)&dir_inr_tab[4]; //注意dir文件排布,选中的dir文件必须是f1x文件
            err = play_one_file(&dec_pctl[0]);
            if (err) {
                play_next_file(&dec_pctl[0]);
            }
